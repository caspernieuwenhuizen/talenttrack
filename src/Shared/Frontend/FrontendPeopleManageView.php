...
class FrontendPeopleManageView extends FrontendViewBase {
    ...
    private static function renderForm( ?object $person ): void {
        ...
        <form id="<?php echo esc_attr( $form_id ); ?>" class="tt-ajax-form" data-rest-path="<?php echo esc_attr( $rest_path ); ?>" data-rest-method="<?php echo esc_attr( $rest_meth ); ?>" data-redirect-after-save="list"<?php if ( $draft_key !== '' ) : ?> data-draft-key="<?php echo esc_attr( $draft_key ); ?>"<?php endif; ?>>
            ...
            $list_url   = add_query_arg( [ 'tt_view' => 'people' ], $dash_url );
            $detail_url = $is_edit ? add_query_arg( [ 'tt_view' => 'people', 'id' => (int) $person->id ], $dash_url ) : $list_url;
            ...
            echo FormSaveButton::render( [
                'label'      => $is_edit ? __( 'Update person', 'talenttrack' ) : __( 'Save person', 'talenttrack' ),
    ...
    private static function renderAssignmentsSummary( int $person_id ): void {
        ...
        foreach ( $assignments as $a ) {
            ...
            $role_label = $role
                ? ( \TT\Infrastructure\Query\LabelTranslator::functionalRoleLabel( (string) ( $role->role_key ?? '' ), (int) $role->id ) ?? (string) $role->label )
