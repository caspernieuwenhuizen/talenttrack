...
final class FrontendMeasurementEntryView extends FrontendViewBase {
    ...
    private static function allTeams(): array {
        ...
        $rows = $wpdb->get_results( $wpdb->prepare(
            "SELECT id, name FROM {$p}tt_teams WHERE club_id = %d AND " . \TT\Infrastructure\Archive\ArchiveRepository::filterClause( 'active' ) . " ORDER BY name ASC",
            CurrentClub::id()
        ) );
    ...
    /**
     * In-body catalogue links: a back-aware "Manage tests" cross-link (so
     * staff move Record → Configure without the dashboard, §5 back-pill via
     * BackLink::appendTo) plus the "+ New test" wizard entry. These are
