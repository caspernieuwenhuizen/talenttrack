...
final class PhasesRepository {
    ...
    public function listForPrimer( int $primer_id, bool $include_archived = false ): array {
        ...
        return (array) $wpdb->get_results( $wpdb->prepare(
            "SELECT * FROM {$t} WHERE primer_id = %d AND club_id = %d{$where} ORDER BY side ASC, phase_number ASC",
    ...
    /**
     * Hard-delete a club-authored phase. Shipped rows are read-only
     * reference content and are never removed. Returns false when the row
     * is shipped or the delete affected no rows (wrong id / other club).
     */
