...
final class FrameworkPrimerRepository {
    ...
    public function activeForClub( ?string $club_scope = null ): ?object {
        ...
        if ( $club_scope !== null && $club_scope !== '' ) {
            $row = $wpdb->get_row( $wpdb->prepare(
                "SELECT * FROM {$t} WHERE club_scope = %s AND archived_at IS NULL ORDER BY id DESC LIMIT 1",
                $club_scope
            ) );
            if ( $row ) return $row;
        }
        $row = $wpdb->get_row(
            "SELECT * FROM {$t} WHERE club_scope IS NULL AND archived_at IS NULL ORDER BY is_shipped DESC, id DESC LIMIT 1"
        );
        return $row ?: null;
    }
    ...
    public function listAll( bool $include_archived = false ): array {
        ...
        return (array) $wpdb->get_results( "SELECT * FROM {$t}{$where} ORDER BY is_shipped DESC, id ASC" );
    }
