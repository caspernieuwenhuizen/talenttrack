...
class ReportsPage {
    ...
    private static function renderReportView( string $report ): void {
        // #1369 — the legacy Player Progress & Radar report is ported
        // to the frontend dashboard (standard report
        // `player-progress-radar`); the wp-admin route redirects there
        // so existing muscle memory + bookmarks keep working.
        if ( $report === 'legacy' ) {
            wp_safe_redirect( add_query_arg(
                [ 'tt_view' => 'standard-report', 'slug' => 'player-progress-radar' ],
