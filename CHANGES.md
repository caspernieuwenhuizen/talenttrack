# TalentTrack v3.109.5 — Session management UI + login-fail tracking (#0086 Workstream B Children 2 + 3)
...
Renumbered v3.109.0 → v3.109.5 mid-rebase as parallel-agent ships took the v3.109.0 (deferred-cleanup), v3.109.1 (#0083 deferred follow-ups), v3.109.2 (seed review Excel export), v3.109.3 (#0078 Phase 2 custom widgets), and v3.109.4 (#0078 Phase 3 builder admin page) slots.
...
1. Removed `FrontendMySessionsView`, `tt-sessions`, and `my_sessions` from the forbidden-token list. Those names belong to the v3.109.5 WP-login-session feature, not the OLD training-session vocabulary the guard was put in place to protect. The remaining tokens still catch the OLD vocabulary regressions.
...
- `languages/talenttrack-nl_NL.po` — 9+ new msgids; v3.108.1's `Head coach` / `Team manager` duplicates removed.
