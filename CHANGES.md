# TalentTrack v4.45.13 — Podium restyled to the 2026 look (#1695)

The **Top performers** (Podium) view body adopts the shared 2026 "chrome" visual language introduced by the global app chrome (#1690). Above the per-team podiums sits a summary KPI strip built from `FrontendAppChrome::kpiTile()` — *Teams ranked* and *Top performers*, both real counts derived from the data already loaded, no extra query. Each team becomes a white chrome card (1px `#e3e6e1` border, 14px radius, `--tt-shadow-md`) with a gradient header carrying the team name (linking through to the team detail), a rounded age-group chip, and — on the leading team only — a brand-gold border accent plus a "Leading" pill so the eye lands on the strongest performers first. The shared `PlayerCardView::renderPodium()` still renders the top-3 player cards inside each card untouched (it's reused by the player and coach dashboards), so only the `FrontendPodiumView` wrapper changed. Styling is a new mobile-first `assets/css/frontend-podium.css` (dep on `tt-frontend-app-chrome`) reading the existing `--tt-primary` / `--tt-secondary` tokens — no new palette, no REST or query change, navigation and breadcrumb contracts intact. Two new strings ("Teams ranked", "Leading"), Dutch added.

# TalentTrack v4.45.13 — Shared frontend app chrome: top bar + persona chip + KPI tile (#1690)
