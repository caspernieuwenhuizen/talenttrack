# TalentTrack v4.45.12 — Goals view restyled to the 2026 look (#1687)

The goals surfaces adopt the 2026 mockup (`marketing/pitch/mockups/05-goals.png`). The player **My goals** view (`FrontendMyGoalsView`) is now a three-column board — Actief / Behaald / Gemist — grouping goals on the same status buckets `GoalsRepository` already uses (completed / cancelled, everything else is active), with each goal rendered as a white chrome card carrying a status chip, a priority chip, a due-date pill, the description, and overlapping player-and-coach owner avatars. The coach goal-detail card (`FrontendGoalsManageView::renderDetail`) gets the matching chrome + chip treatment in place of its inline-styled definition list. Styling lives in a new mobile-first `assets/css/frontend-goals.css` (single stacked column at ~360px, three columns at ≥768px) that reads the existing `--tt-primary` / `--tt-secondary` tokens — no new palette — and is enqueued with the global app-chrome sheet as its dependency; the change is presentation-only, touching no queries, REST, breadcrumb/back navigation, or Save/Cancel chrome. Three new strings (Achieved / Missed / Nothing here yet.), Dutch added. Patch bump. (#1687)

# TalentTrack v4.45.12 — Shared frontend app chrome: top bar + persona chip + KPI tile (#1690)
