# TalentTrack v4.45.11 — Scouting & onboarding pipeline restyled to the 2026 look (#1689)

The standalone scouting / onboarding pipeline — the funnel at `?tt_view=onboarding-pipeline` rendered by `FrontendOnboardingPipelineView` — adopts the 2026 design on its existing six real stages (Prospects, Invited, Test training, Trial group, Team offer, Joined). Each column header keeps its count as a rounded chip and now stacks compact white player mini-cards: an initials avatar beside the prospect's name and meta line, an amber "Action needed" chip on overdue early-funnel cards, and a green "Player" chip on cards that have reached the Joined lane. Styling is the restyled mobile-first `assets/css/components/onboarding-pipeline.css` (now depending on the shared `tt-frontend-app-chrome` stylesheet) reading the existing brand tokens, with no new palette; the funnel is a single vertical scroll at 360px and the side-by-side six-column layout on tablet/desktop. No query, stage, or REST change — same data, new look.

# TalentTrack v4.45.11 — Shared frontend app chrome: top bar + persona chip + KPI tile (#1690)
