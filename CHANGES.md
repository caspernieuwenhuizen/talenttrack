# TalentTrack v4.45.20 — Methodology: Vision-first tabs, collapsible Framework sections, 2026 restyle (#1671)

The Methodology library view (`?tt_view=methodology`) adopts the 2026 chrome look and a development-first reading order. (A) The tab pills now lead with **Visie** (the club's playing vision) before **Raamwerk → Formaties → Spelprincipes → Voetbalhandelingen → Spelhervattingen**, and Visie is the default landing tab — a coach opening the methodology sees *where the academy is going* first, not the framework reference. The lead tab carries a subtle gold ring; the active tab is brand green. (B) The Framework (Raamwerk) tab's seven stacked sections (Voetbalmodel, Voetbalhandelingen, Vier fasen, Leerdoelen, Factoren van invloed, Reflectie, De toekomst) are now native `<details>` accordion cards — the first opens by default, the rest are collapsed, and each section's open/closed state persists across visits via `localStorage` keyed by a stable id (`assets/js/methodology-accordion.js`). Summaries are ≥48px keyboard-operable tap targets with `:focus-visible` rings. (C) The styling moved out of the inline `<style>` block into a new mobile-first `assets/css/frontend-methodology.css` (enqueued with the `tt-frontend-app-chrome` dependency so it inherits the brand tokens): tabs and accordion blocks are white cards with a numbered index chip and a chevron that rotates on open, using `--tt-primary` / `--tt-secondary` / `--tt-muted` / `--tt-shadow-md` only. The §5 breadcrumb/back contract is untouched. One new Dutch string. Patch bump. (#1671)

# TalentTrack v4.45.20 — Shared frontend app chrome: top bar + persona chip + KPI tile (#1690)
