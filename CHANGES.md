# TalentTrack v4.45.15 — My team restyled to the 2026 look (#1695)

The player/parent-facing **My team** view body (`src/Shared/Frontend/FrontendMyTeamView.php`) now speaks the same 2026 "chrome" visual language as the global app chrome (#1690). The three sections — the team podium, the viewer's own player card, and the teammate roster — are each framed as a white card (1px `#e3e6e1` border, 14px radius, `var(--tt-shadow-md)` lift), with the podium card carrying a gold top accent that ties the headline back to the brand band above it. The opt-in rank badge (#1384) and the growth-trend chip now read the brand tokens (`--tt-primary` / `--tt-secondary`) instead of one-off greys, and the teammate roster moves from a hand-rolled inline-styled flex wrap to a responsive `minmax()` grid of brand-green avatar tiles, each a ≥48px touch target with a `:focus-visible` / hover affordance. The old inline `<style>` block and the per-element inline styles are gone, replaced by a new mobile-first sheet `assets/css/frontend-my-team.css` (base 360px, scaling up at 768px) that depends on `tt-frontend-app-chrome`. Nothing about the data changes: the same real podium / rank / trend / roster data renders, the player card and podium components (`tt-pc*`) keep their own styling, navigation stays at the two canonical affordances (CLAUDE.md §5), and there are no new user-facing strings — this is presentation only.

# TalentTrack v4.45.15 — Shared frontend app chrome: top bar + persona chip + KPI tile (#1690)
