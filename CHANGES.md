# TalentTrack v4.45.14 — My evaluations restyled to the 2026 look (#1695)

The player/parent-facing "My evaluations" body (`FrontendMyEvaluationsView`) adopts the shared chrome visual language from #1690: each evaluation now renders as a white card (1px `#e3e6e1` border, 14px radius, `--tt-shadow-md`) with a warm header strip, the coach-feedback message reads as a gold-accented callout, and the subcategory disclosure is a rounded chip-style toggle that meets the 44px touch target. A summary KPI row at the top reuses `FrontendAppChrome::kpiTile()` for two real stats — the total evaluation count and the latest overall rating with an up/down/flat trend chip diffing the previous cut — both composed from data already fetched, so there is no new query and no REST change. A new mobile-first `assets/css/frontend-my-evaluations.css` (enqueued with a dep on `tt-frontend-app-chrome`) reads the existing brand tokens; no new palette, no markup-meaning or nav changes. This is a tasteful restyle, not a redesign.

# TalentTrack v4.45.14 — Shared frontend app chrome: top bar + persona chip + KPI tile (#1690)
