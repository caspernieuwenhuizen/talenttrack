...
# TalentTrack v4.127.0 — Demo PDP dossiers are only closed once their conversations have been held (#3650)

A generated academy stamped roughly three in ten of the current season's PDP
dossiers *completed*, with a signed-off end-of-season verdict, while all four
of their conversations were still in the diary. Reading the demo, a head of
development saw a finished season in the week it started, and PDP coverage
reported everybody covered while almost no conversation had taken place. A
dossier is now closed by its last conversation rather than by chance: early in
a season the demo shows open cycles, which is what that season honestly looks
like. Finished seasons still close every dossier with a verdict, unchanged.

# TalentTrack v4.127.0 — Planned attendance over REST no longer reads as a taken register (#3652)
...
# TalentTrack v4.127.0 — Players and parents can see their own playing time (#3666)
...
# TalentTrack v4.127.0 — A new PDP cycle plans its talks at 18:00, not at 07:59:59 (#3670)
...
# TalentTrack v4.127.0 — Date notation: "System default" follows the site language (#3680)
...
# TalentTrack v4.127.0 — Match prep starts from the match length set on the activity (#3682)
...
# TalentTrack v4.126.3 — Demo academies train on Tuesday and Thursday and play on Saturday, with times (#3660, #3676)
...
# TalentTrack v4.126.3 — Head of Development can update and print any player's PDP conversations (#3663)
...
# TalentTrack v4.126.3 — Match execution spots a half that was left running (#3667)
...
# TalentTrack v4.126.3 — REST: the PDP files list honours a plain team_id (#3668)
...
# TalentTrack v4.126.3 — Lists no longer say "nothing recorded yet" before they have loaded (#3669)
...
# TalentTrack v4.126.3 — Training run explains the player notes sheet before attendance is taken (#3671)

The sideline view of a training no longer hides "Notes on players" until someone is on the register. Before attendance is taken it now shows the heading, a line saying notes open up once attendance is taken, and a "Take attendance" button to this training's register (or to the training itself when the register grid is off). The overview before you start the training also names every block, in a list under the timeline strip and as a label on each segment, so a coach can tell which segment is the rondo.

# TalentTrack v4.126.3 — The activity peek only shows activities you may open (#3688)
...
# TalentTrack v4.126.3 — One body contract for REST write routes (#3689)
...
# TalentTrack v4.126.3 — Match prep's unknown-field error lists the fields it accepts (#3690)
...
# TalentTrack v4.126.3 — Training observations only for players who were there (#3694)
...
# TalentTrack v4.126.0 — The monthly report can carry per-section settings (#3514)

Groundwork, invisible on screen until the next release in this series.

A monthly report was composed by ticking sections on or off, and everything
else about a section was decided for you. Asking for *one* of the tests you
ran, or for the readings rather than the summary, could not be expressed —
not in the link you copy, not in a saved view, not in a monthly schedule.

A composition now carries settings per section alongside the section list, and
they travel everywhere a composition already travelled: the address in your
browser, your saved views, the PDF link, and the monthly schedule's own copy.

Nothing changes in any existing report. A composition with no settings — which
is every one saved so far — renders exactly as it did, and a view saved before
this release still opens.

# TalentTrack v4.126.0 — Monthly report: choose which tests to show, and how much of each (#3515)
...
# TalentTrack v4.126.0 — Monthly report: a match results and statistics section (#3516)
...
# TalentTrack v4.126.0 — Monthly report: meeting snapshots, with notes per section (#3517)

The monthly report shows current data, which a staff meeting cannot run on: open
it on the 3rd, discuss it on the 5th, and a register taken in between has moved
the numbers under the discussion.

**Save snapshot for the meeting** freezes what you are looking at. A snapshot
keeps the numbers as they were and has its own address, so reopening it next
season shows what the meeting actually saw. Each section can carry one typed
note, and those notes stay editable after the snapshot is taken while the numbers
do not — you draft before the meeting and write down what was decided during it.
Every note records who last wrote it and when, and the PDF prints them, so the
printed copy and the screen say the same thing.

**A snapshot cannot be shared by link.** It is readable only by someone signed in
who can already see that team's reports — no share token, no public address. This
is deliberately different from match prep and match analysis: a monthly report
names every player in the squad alongside their attendance, their test results
and who needs a conversation, and a link that works for anyone holding it is the
wrong shape for that. Staff who do not log in get the PDF, handed over by someone
who does.

# TalentTrack v4.126.0 — Monthly report: player tables read in squad-number order (#3518)

The team monthly report now lists players by squad number — on screen and on the
printed copy — so a player sits in the same place in every table instead of
moving between the roster, the attendance list and the test results. Players
without a squad number come last, alphabetically.

The two tables where the order carries the finding are unchanged: minutes share
still reads by share played, and the players needing a conversation still read
most urgent first.

# TalentTrack v4.126.0 — A team's record, form and leaderboards as one reader (#3520)
...
# TalentTrack v4.126.0 — The team page splits into Overview and Statistics (#3521)

The team page stacked eight panels on one scroll — key facts, at a glance,
roster, staff, team info, trial roster, upcoming activities, media — and had
nowhere left to put a ninth.

It now carries two tabs under the team's name. **Overview** is the page exactly
as it was, in the same order, still governed by each coach's own Customize
choice. **Statistics** is where a team's record, form and leaderboards land in
the next release; until then it says so rather than opening onto a blank panel.

Two tabs, not one per panel: the section list stays under Customize, where it
has always been, because eight tabs for eight cards would read worse than the
scroll they replaced.

Switching tabs reloads the page, which is the point — opening a roster no longer
has to pay for statistics nobody asked for. An old bookmark with no tab, or an
unrecognised one, opens Overview, and the "back to…" link you arrived with
survives the switch.

The pinned team name moved up the page to carry the tabs. It now stays with you
from the top of the page rather than arriving halfway down it.

# TalentTrack v4.126.0 — The team page's Statistics tab is filled in (#3522)
...
# TalentTrack v4.126.0 — The test environment runs a pinned WordPress version (#3525)
...
# TalentTrack v4.126.0 — The Metingen tab is one register per category (#3526)
...
# TalentTrack v4.126.0 — The date notation setting now reaches every screen (#3528)
...
# TalentTrack v4.126.0 — Record a match result without the live match sheet (#3530)
...
# TalentTrack v4.126.0 — The minutes grid can record the score (#3531)
...
# TalentTrack v4.126.0 — Tournament fixtures can record their results (#3532)

A tournament day with five fixtures had nowhere to record any of their results.
The table behind it carried the opponent, the formation, the duration and the
substitution windows — and no score at all, so a tournament read in the product
as attendance and minutes with no outcome attached.

Each fixture in the match programme now carries **Ours** and **Theirs**, saved
as you move off the box.

This is deliberately *not* the single scoreline a league match has. A tournament
day is several matches and one score cannot describe it, which is why a
tournament still gets no Result card and no score boxes in the minutes grid. The
result belongs to the fixture, so that is where it lives.

Leaving a box empty records **no result**, not 0–0 — a fixture you have not
played, or one nobody typed a score into, reads as played-without-a-result.

There is no day total. The team's record leaves tournaments out entirely, so a
goals-for/against across the day would have nothing reading it yet.

Goals scored at a tournament already counted for the player who scored them, and
still do; that half never depended on these boxes.

# TalentTrack v4.126.0 — The demo academy now has a frozen meeting report (#3539)
...
# TalentTrack v4.121.0 — Filter chips: named, removable, and the same everywhere (#3346)
...
# TalentTrack v4.121.0 — Player comparison: filters apply in place, and the surface stops emitting invalid HTML (#3352)
...
# TalentTrack v4.121.0 — Deleting an activity now cleans up after itself (#3376)
...
# TalentTrack v4.121.0 — PDP conversations: the retired agenda column is gone (#3381)
...
# TalentTrack v4.121.0 — Message log: a row now says whether the recipient was reachable, beside why the send stopped (#3383)
...
# TalentTrack v4.121.0 — An invitation that was never sent now says so (#3387)
...
# TalentTrack v4.121.0 — Demo evaluations: four rounds a season instead of two a week (#3401)
...
# TalentTrack v4.121.0 — Demo data: a season per year the window covers (#3402)
...
# TalentTrack v4.121.0 — Demo history window: three years, and a clamp that says so (#3403)

The per-run **weeks of history** ceiling goes from 104 to 156 — three years
rather than two, which is a full academy arc and what the new multi-season
shape and four-rounds-a-season cadence were built for. The teams and
players-per-team ceilings are unchanged; nothing argued for more of either.

Asking for more than a ceiling allows no longer reduces the number in silence.
The run goes ahead with the maximum and reports it: a `WP_CLI::warning()` on
the command line and a notice on the demo-data page, each naming the field, the
number asked for and the number used. Refusing outright would be worse — an
operator who asks for too much should still get a run, just an honest one.

# TalentTrack v4.121.0 — Potential overview: a squad's potential bands, in one place and editable (#3412)
...
# TalentTrack v4.121.0 — The status dot now says when it was computed without potential (#3413)
...
# TalentTrack v4.121.0 — The KPI snapshot now carries the potential band (#3414)
...
# TalentTrack v4.121.0 — Safeguarding broadcast: the message the product promised but could not send (#3423)
...
# TalentTrack v4.120.1 — Migrated player photos record their real size and fingerprint (#3421)
...
# TalentTrack v4.120.0 — A player's own profile is a screen they can use (#3393)
...
# TalentTrack v4.120.0 — A teammate's card is no longer a one-way door (#3395)

Tapping a teammate on **My team** opened a card with no breadcrumb trail and no
back link — the browser's back button was the only way out. Every other player
screen has carried the trail for releases; this one was listed in the standards
as a sub-view while being reachable by its own address.

The teammate card now shows the trail back to the dashboard, and arriving from
My team — from the roster or from the podium — offers a **← Back to My team**
link as well.

# TalentTrack v4.120.0 — A goal waiting on your coach no longer looks like one you should be working on (#3396)
...
# TalentTrack v4.120.0 — The goals on My PDP open now (#3397)
...
# TalentTrack v4.120.0 — My journey now speaks to the player reading it (#3398)

The journey screen backs two addresses — a coach opening a player's timeline,
and the player opening their own — and rendered the same text to both. A player
saw their own name in the third person under a heading that said "Mijn reis", a
description of the page as a story "for this player", and a pointer to "the
parent meeting", which is a staff appointment they do not attend.

The player now gets their own heading and description. When entries on their
timeline are visible only to coaching staff, the count still appears — a
timeline with silent gaps would be its own kind of dishonesty — but it now says
plainly who can see them and that their coach is the person to ask. The three
always-visible filters are theirs too: what a coach wrote, what they were set to
work on, and moving up an age group, in place of a trials filter that is empty
for a signed player.

The welcome card a parent sees also said "you choose what your child shares with
you". The player chooses; it now says so.

# TalentTrack v4.120.0 — Player photographs are no longer public files (#3399)
...
# TalentTrack v4.120.0 — Demo players now have an age-group history (#3404)
...
# TalentTrack v4.117.0 — PDP preparation: configurable question sets per conversation (#3305)
...
# TalentTrack v4.117.0 — VCT match-day context works again, and the fixture peeks fill in (#3355)
...
# TalentTrack v4.117.0 — Analytics reports name the activity again instead of showing a bare id (#3356)
...
# TalentTrack v4.117.0 — A three-week reference phase profile, and the groundwork for per-team cycles (#3357)

The VCT configuration screen's reference profiles gain a three-week option
alongside the existing four-week, six-week and five-week speelwijze cycles. It
runs introduce → load → unload: three weeks leaves no room for a peak week, so
it is a different shape rather than a shortened six-week ladder.

Behind it, the schema for per-team repeating cycles — a cycle length and start
week per team, and a place to record a week a coach has set to neutral by hand.
Nothing reads them yet; configuring a cycle arrives with the screen that edits
one. An academy that never sets a cycle up plans exactly as it does today.

# TalentTrack v4.117.0 — The cycle knows which week a team is in, and pauses for a game (#3358)

The rule behind per-team cycles: a week in which the team plays is neutral, and
that week is not spent. A team in week 3 of its cycle that plays on Saturday
stays in week 3 the following week — the cycle stretches around the fixture list
instead of drifting out of step with it. Games and tournaments both count; a
cancelled game does not.

A coach can also set a week neutral by hand, or force a week to run normally
despite a fixture, for the friendly that was never meant to interrupt anything.

Nothing surfaces this yet — the planner starts using it next, and the screens to
configure and correct a cycle follow.

# TalentTrack v4.117.0 — Drafted trainings follow the team's cycle, and say when the cycle is paused (#3359)

Where a team has a cycle, the planner now takes the week's intensity from it
rather than from the macro-block alone. A week with a game is planned at neutral
intensity, and the draft says why — "there is a game this week, so the cycle is
paused" — instead of quietly coming out flat.

A team with no cycle plans exactly as it did before.

One long-standing silence also gets a voice: a macro-block that runs longer than
its weekly profile has always planned its later weeks at neutral intensity
without saying so. It still does, but now it tells you the profile ran out and
that adding weeks to it keeps the progression going.

# TalentTrack v4.117.0 — Give a team a training cycle of three, four or six weeks (#3360)
...
# TalentTrack v4.117.0 — See the cycle week by week, and correct any week (#3361)
...
# TalentTrack v4.117.0 — A training says which week of the cycle it is (#3362)
...
# TalentTrack v4.116.0 — One period control on the reports and grids, not two (#3331)
...
# TalentTrack v4.116.0 — Filtering the audit log no longer reloads the page (#3336)
...
# TalentTrack v4.116.0 — The attendance and minutes grids filter without reloading — and without losing your work (#3337)
...
# TalentTrack v4.116.0 — The attendance and minutes reports filter without reloading (#3338)
...
# TalentTrack v4.116.0 — The message log and alerts inbox filter without reloading (#3339)
...
# TalentTrack v4.115.6 — The player header shows every position, not just the first (#3329)

A player with four preferred positions showed one pill in the header while
the Identity row, the sidebar card and the edit form all listed four — so a
coach reading only the header saw a specialist where the record says the
player covers half the pitch.

The header now carries one pill per position, using the short code you have
set for it, with the full name on hover. Positional range is part of who a
player is in the squad, and the header is where that picture forms.

# TalentTrack v4.115.6 — Taking one filter off a list works, and the comparison view can be cleared (#3333)
...
# TalentTrack v4.115.5 — The match sheet is one landscape page again (#3297)

The exported match-prep PDF was readable but ran to two pages — a touchline
sheet you have to turn over is a sheet you lose. It is one page now, laid out
for paper rather than for the screen: selection with the roles underneath it
on the left, the two half-pitches and the match goals down the middle, and
Doen per speler filling the right-hand column.

That block now always reserves fifteen rows — the players who already carry a
note, then ruled lines to the count — so the room you write in during the
warm-up is there whether or not you filled it in beforehand. Empty goal boxes
print as writing lines too, and a full 22-player squad still fits the page.

The Print match sheet route prints the same layout, so the page you print and
the PDF you export are finally the same document.

# TalentTrack v4.115.5 — You can save a view you built from the dropdowns (#3327)

Narrowing a list with its Team, Position or Archive dropdowns gave you no way
to save that view — the bookmark control simply was not there. Typing
something into the search box made it appear, which made the whole feature
look intermittent.

The control now appears whenever anything is filtered, however you filtered
it, and a list you open on a saved view says so again.
