# TalentTrack v4.45.2 — Team attendance report restyled to the 2026 look (#1688)

The team-attendance report (`FrontendAttendanceTeamReportView`) adopts the 2026 chrome look introduced in #1690. Above the table sits a KPI summary strip built from the shared `FrontendAppChrome::kpiTile()` — team count, total activities, average attendance across all teams, and a count of teams below 70% that flags red when any team is under threshold. The per-team table is wrapped in a rounded card, and the Present % cell now carries an inline attendance bar that fills proportionally and turns red below 70% (the bar track drops on narrow screens, keeping the number). This is a visual-only restyle: the underlying per-team query and data are unchanged, the KPI figures are presentation-level aggregations of the rows already fetched.
