# TalentTrack v4.45.17 — Match-execution restyled to the 2026 look (#1684)

The assistant-coach live-match surface (`FrontendMatchExecutionView`) — the phone-first tracker the coach runs on the sideline — adopts the 2026 "chrome" design on its real sections, on real data. The **score** becomes a dark-green/gold live band; the **timer** carries a gold-on-green clock chip with the live pulse dot; the **tracked-players**, **bench**, and **sub-target** sections render as white brand-token cards (1px `#e3e6e1`, radius 14px, `var(--tt-shadow-md)`); action buttons and the goal/score chips pick up the brand palette; and a two-up **KPI summary strip** (*Tracked players* · *Available squad*) renders through the shared `FrontendAppChrome::kpiTile()` (#1690), mirroring the mockup's "Minuten gelogd" footer with the real counts the view already computes.

This is purely a presentation layer. A new mobile-first `assets/css/frontend-match-execution-2026.css` layers **additively** over the existing `frontend-match-execution.css` (enqueued with a dependency on `tt-frontend-app-chrome`, so brand tokens + KPI styles are present and the additive rules win without `!important`). Every live-JS hook is preserved verbatim — the timer toggle, score steppers, per-player goal counters, substitution flow, post-match/late-event panels, and the offline queue all keep their existing classes, ids, and `data-*` attributes; the restyle only adds brand styling to the markup that's already there. No query, REST, or state-machine change (CLAUDE.md §4). Two new strings (*Match summary*, *Available squad*), Dutch added. Part of the 2026 per-view parity epic (#1695). Patch bump.

# TalentTrack v4.45.17 — Shared frontend app chrome: top bar + persona chip + KPI tile (#1690)
