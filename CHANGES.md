# TalentTrack v4.45.16 — My journey restyled to the 2026 look (#1695)

The player- and parent-facing **My journey** surface — and the coach-side player-journey view, both rendered by `FrontendJourneyView` (`src/Modules/Journey/Frontend/FrontendJourneyView.php`) — adopts the 2026 "chrome" visual language introduced with the shared app chrome (#1690). The chronological journey becomes a clean left-rail **timeline**: brand-coloured nodes (green/gold, taken per event type from the event-type registry) sit on a continuous rail, each row a white **chrome card** (1px `#e3e6e1` border, 14px radius, `--tt-shadow-md` shadow, per-event accent edge). The event-type filters become pill chips, and the severity tags read as gold "Milestone" / soft-red "Warning" pills. All presentation moves out of inline `style="…"` attributes into a new mobile-first `assets/css/frontend-my-journey.css`, which depends on `tt-frontend-app-chrome` and reads the existing `--tt-primary` / `--tt-secondary` tokens — no new palette. Mobile-first per CLAUDE.md §2: base ~360px, ≥44–48px tap targets, no hover-only behaviour, `.tt-`-prefixed scoped selectors. This is presentation only — no query, REST, data model, or permission changes; the same events render, the navigation contract (breadcrumbs + `tt_back` pill) and the timeline/transitions toggle are untouched.

# TalentTrack v4.45.16 — Shared frontend app chrome: top bar + persona chip + KPI tile (#1690)
