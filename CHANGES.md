...
6. **`tt_vct_session_blocks`** — filled slots per VCT session. **No DB CASCADE** — `VctSessionsRepository::delete()` (VCT-5) will do child cleanup in a transaction; integration test (VCT-5) will assert no orphans. `exercise_id` nullable so a coach can hand-fill a custom block. FK column is **`vct_session_id`**, not `session_id` (the spec uses `session_id` for brevity, but `session_id` is the legacy training-session FK token banned under the #0035 no-regression rule — the disambiguated name is also self-documenting since VCT sessions are a distinct concept from both the renamed-away `tt_sessions` entity and from WP login sessions). UNIQUE / index renamed to match.
