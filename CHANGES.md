# TalentTrack v4.45.18 — Match-prep restyled to the 2026 look (#1695)

The head-coach match-preparation view (`?tt_view=match-prep`) body adopts the 2026 "chrome" visual language established by the shared app-chrome work (#1690), as the first per-view slice of the visual-parity epic (#1680). The toolbar, the roster / player-focus / roles panels, the match-goals panel and each goal box, and the half-pitches now render as white 2026 cards (1px `#e3e6e1` rims, 12–14px radius, `var(--tt-shadow-md)`); the role pickers and availability status buttons become 2026-rounded chips; and inputs gain a brand-green focus ring. Everything reads the brand tokens from `public.css` (`--tt-primary` `#0b3d2e`, `--tt-secondary` `#e8b624`, `--tt-ink`, `--tt-muted`) — no new palette. The legacy `--tt-mp-*` variables are re-pointed at the brand palette so the existing rules pick up the new look; the pitch tints keep their 1e/2e half meaning.

A new at-a-glance **KPI strip** sits at the top of the surface, built entirely from data the view already loads: **Available** players, **Unavailable** players (flagged red when any), **1st-half starters** assigned (n/11, flagged green at 11), and the chosen **Formation**. Tiles are rendered with the shared `FrontendAppChrome::kpiTile()`; the aggregation lives in a pure `summaryCounts()` helper (no query, no eligibility decision) so it stays SaaS-portable (CLAUDE.md §4). The strip is hidden on print.

Visual layer only: a new mobile-first `assets/css/frontend-match-prep-2026.css` (enqueued with dep `['tt-frontend-app-chrome','tt-match-prep']`) sits on top of the existing match-prep sheet, which still owns the spreadsheet grid geometry, the pitch SVGs, the picker/drawer overlays, and the entire `@media print` block — all untouched. No query, REST, or behaviour change; the navigation contract (§5) and the wizard hand-off are unchanged. Four new strings ("Match prep summary", "Available", "Unavailable", "1st-half starters"), Dutch added.

# TalentTrack v4.45.18 — Shared frontend app chrome: top bar + persona chip + KPI tile (#1690)
