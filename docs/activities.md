...
- **Facts strip** — a few quick facts. A training shows Date · Time · Presence time · Type · Status; a match day shows Date · Opponent · Home/Away · Result · Presence time · Kick-off · End time · Formation. Facts with no value are left out. Every activity date here carries its weekday — `Fri 11-09-2026` — because a coach reads a fixture by the day it falls on.

 The match day's three times read in the order the day happens: **Presence time** (when to be there), **Kick-off**, then **End time**. All three come straight from what was saved on the activity, so a presence time entered on the edit form is visible to everyone who opens the activity, not only to whoever typed it. A friendly or a tournament shows its presence time alongside the Time range instead — and so does any other activity that has one stored.
- **Cards** — each with a titled header, only the cards that have something to show appear, so the page stays uncluttered:
 ...
 - **Attendance** (completed activities) — a breakdown bar and legend across Present / Absent / Late / Excused / Injured (plus any custom statuses), with the headline `X / Y present (Z%)` linking to the attendance edit form. Under the bar, a muted line reads **Register last saved by <name> on <date>**. A note warns when roster players still have no attendance row. A collapsible **Show roster** list underneath names each registered player with their status pill (guests tagged), so you can see who had which status without opening the edit form.

 That line says who saved the **register**, not who set each individual mark. Saving a register rewrites all of its rows at once, so one save stamps them all with the same name and time — if three people each corrected one player, you see the last of the three. It answers "who wrote this down, and when", which is the question you have when a mark turns up that you didn't make. Registers recorded before this was introduced carry no name, and the line stays away rather than guessing.
...
Two things this does not do. A roster you supply yourself is kept exactly as you gave it — nobody is added to it. And a squad you have **trimmed stays trimmed**: the roster is filled in only when it is empty, so a player you deliberately took off does not come back the next time anything touches the activity.
