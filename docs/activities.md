...
The flag says only that the player cannot be planned for. It carries **no medical detail at all** — no injury type, no body part, no note, no dates — so a coach or assistant who has no access to injuries can still plan around it without seeing a child's medical record. An injury stops flagging as soon as a return date is recorded, and an old record whose expected return has already passed does not flag at all.
