...
| Match execution — clock (#3553) | `POST /match-execution/{activity_id}/start-half` `{half}`, `POST .../end-half` `{half}`, `POST .../pause` `{half}`, `POST .../resume` `{half}`. The server keeps the clock: `pause` stamps `clock_paused_at` (UTC), `resume` adds the gap to the running half's pause total and clears the stamp, `end-half` closes an open pause first, and `start-half` starts the half running. Pausing a paused clock keeps the first stamp and resuming a running one does nothing, so an offline-queue replay cannot double-count. `resume` still accepts the old client-computed `pause_seconds` and ignores it. `start-half`, `pause` and `resume` answer `clock: {half, elapsed_seconds, running}` — seconds into the current half net of pauses, the same answer the live screen boots from, so a reload resumes the match where it is. Gated on `tt_edit_activities`. | `MatchExecutionRestController.php`, `Domain/MatchClock.php` |
