...
| People (staff)   | `GET/POST /people`, `PUT/DELETE /people/{id}`. **Body contract (#3816):** both writes take `first_name`, `last_name`, `email`, `phone`, `role_type`, `wp_user_id` and `status`, and refuse anything else with `400 unknown_field` (`details.fields` + `details.allowed`). The two names are required on `POST` and are named in `details.fields` of a `400 missing_fields`. On `PUT` **every field is partial**: one left out keeps its stored value. That is a fix, not just a declaration — `extract()` wrote both name columns on every call, defaulted to `''`, so a `PUT` carrying only a phone number erased the person's name | `PeopleRestController.php`                              |
