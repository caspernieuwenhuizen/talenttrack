...
| Scouting visits (#3600) | `POST /scouting-visits` (create), `POST /scouting-visits/{id}` (update, only the fields sent), `DELETE /scouting-visits/{id}` (archive). The routes declare their args: `visit_date` (`YYYY-MM-DD`) and `location`, both required on create, plus `visit_time` (`HH:MM`), `event_description`, `age_groups_csv` (comma-separated, e.g. `u13,u14`), `notes`, `status` (`planned` / `completed` / `cancelled`) and `scout_user_id` (defaults to the caller). Writes gate on `tt_edit_prospects`. A scout edits and archives only their own visits, `tt_manage_prospects` any. A prospect is linked to the visit it was found at through `scouting_visit_id` on `PATCH /prospects/{id}`, or by logging it from the visit (`from_visit` on the new-prospect wizard). | `src/Modules/Prospects/Rest/ScoutingVisitsRestController.php` |
