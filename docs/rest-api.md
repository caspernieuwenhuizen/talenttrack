...
| Test trainings (#0081, #3818) | `POST /test-trainings` creates a session for prospects to be seen at, and `DELETE /test-trainings/{id}/permanent` erases one (gated on `tt_manage_recycle_bin`, like every other purge). The create declares its args: required `date` (`YYYY-MM-DD` or `YYYY-MM-DD HH:MM:SS`; a bare date is stored at 18:00), plus `location`, `age_group_lookup_id` (omit for a mixed-age session), `coach_user_id` (defaults to the caller) and `notes`. A key outside that set is `400 unknown_field` with `details.fields` and `details.allowed`, before anything is written — `age_group` and `coach` are the two that used to vanish into an empty-looking session. Writes gate on `tt_edit_prospects`, `tt_manage_prospects` or `tt_edit_settings`. There is deliberately no list route: the sessions are read through the onboarding-pipeline surfaces. | `src/Modules/Prospects/Rest/TestTrainingsRestController.php` |
