...
| Test trainings (#0081, #3818) | `POST /test-trainings` creates a test training for prospects to be seen at, and `DELETE /test-trainings/{id}/permanent` erases one (gated on `tt_manage_recycle_bin`, like every other purge). The create declares its args: required `date` (`YYYY-MM-DD` or `YYYY-MM-DD HH:MM:SS`; a bare date is stored at 18:00), plus `location`, `age_group_lookup_id` (omit for a mixed-age one), `coach_user_id` (defaults to the caller), `notes` and `prospect_id` (#3932). `prospect_id` is not a column: passing it records that the prospect was invited to this session, through the same completed `invite_to_test_training` task the workflow route writes, and the response carries the `task_id` it wrote. It needs `tt_invite_prospects` and sight of that prospect (`403 cannot_arrange` otherwise) and consent on record (`409 no_consent`), both checked **before** the session row is written so a refusal leaves nothing behind. A key outside that set is `400 unknown_field` with `details.fields` and `details.allowed`, before anything is written — `age_group` and `coach` are the two that used to vanish into a half-empty record. Writes gate on `tt_edit_prospects`, `tt_manage_prospects` or `tt_edit_settings`. There is deliberately no list route: test trainings are read through the onboarding-pipeline surfaces. | `src/Modules/Prospects/Rest/TestTrainingsRestController.php` |
