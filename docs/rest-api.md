...
- **The gate.** `php tools/check-rest-args.php` (CI: `rest-args-lint.yml`) fails a PR that adds a write route without `args`. Routes that predate the rule are listed in `tools/rest-args-baseline.php`, one `file | route path | methods` per line. When you give a baselined route its `args`, delete its line in the same PR; the gate fails until you do and prints the exact line to delete. Never add a line to make a new route pass. A route whose path or methods it can not read statically (a local variable, a function call) counts as a violation; use a string literal or a `self::` string constant.
