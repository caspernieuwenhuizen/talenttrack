...
**System default follows your site language when the WordPress date format was never changed.** WordPress writes that format once, when the site is installed, in the install language — so a site installed in English and later switched to Dutch kept the English order and printed Dutch month names in it ("oktober 6, 2026"). On **System default**, an untouched WordPress date format is now read as "whatever WordPress writes in this language", which gives "6 oktober 2026" on a Dutch site and leaves an English site exactly as it was. If you *did* set the WordPress date format yourself, that choice is kept as-is — it's a deliberate one. Pick any other preset to decide the order yourself regardless of language.
