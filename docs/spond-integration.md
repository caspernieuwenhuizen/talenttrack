...
- **Activity type** — TalentTrack's keyword classifier picks training, game, tournament, or meeting. **The title decides.** Most keywords match anywhere in the title, so "Thuiswedstrijd" and "Trainingswedstrijd" both read as games. The two short ones that also live inside longer Dutch words — `kamp` (Norwegian for *match*, since Spond is a Norwegian product) and `uit` (away) — only count as **whole words**: "Kamp mot Rosenborg" is a game, while "Trainingskamp" and "Voetbalkamp" are trainings.

  The **description is a fallback, not a second title**. It is read only when the title contains no recognised keyword at all. So a training called "Training JO14-1" whose description reads *"laatste training voor de wedstrijd van zaterdag"* stays a **training** — writing a sentence about the weekend does not turn Wednesday's session into a fixture. An event called just "JO14-1" whose description reads *"wedstrijd tegen Ajax"* does become a game, because nothing in the title said otherwise. The same whole-word rules apply in the description.
