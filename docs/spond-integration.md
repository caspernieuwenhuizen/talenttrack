...
- **Activity type** — TalentTrack's keyword classifier picks training, game, tournament, or meeting from the event title. Most keywords match anywhere in the title, so "Thuiswedstrijd" and "Trainingswedstrijd" both read as games. The two short ones that also live inside longer Dutch words — `kamp` (Norwegian for *match*, since Spond is a Norwegian product) and `uit` (away) — only count as **whole words**: "Kamp mot Rosenborg" is a game, while "Trainingskamp" and "Voetbalkamp" are trainings. A title with no recognised keyword falls back to training. If a coach changes the type later, the system preserves that change across future syncs.
