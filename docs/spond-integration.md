...
- **Location (venue + address)** — when a Spond event's location has both a venue name and a street address, TalentTrack keeps **both** — the venue name on the first line, the address on the second. (The weekly planner PDF prints the venue inline and the address on the line below.) Because location is a "Spond wins" field, editing it in TalentTrack is overwritten on the next sync; the edit form's single-line field also can't hold the second line, so re-saving collapses it — the next sync restores both lines.
