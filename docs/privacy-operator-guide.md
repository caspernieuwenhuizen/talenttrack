...
4. For genuine erasure, hard-delete the player (Players list → bulk actions → Delete permanently, administrator-only; an automatic backup snapshot is taken first). The hard-delete cascades across **every** player-keyed table — evaluations and their ratings, goals including their discussion threads, journey events, injuries, attendance, PDP files with conversations and verdicts, trial cases with staff inputs, match-day and tournament references, reports, behaviour/potential records. Match score events and the prospect-conversion link keep their rows but clear the player reference. Two things stay manual: the player photo (a URL into `wp-content/uploads/` — remove the file via the Media library) and any linked WP user account (wp-admin → Users). Document the erasure decision and date.
5. Log the request and your response in your academy's privacy register.
