...
- **Fire from the repository, not the caller (#3130).** Where several surfaces write the same record, the `do_action` belongs in the repository method they all reach. `tt_trial_started` was fired by three of `TrialCasesRepository::create()`'s four callers, so a player whose trial was opened through the new-player wizard had no trial on their timeline while an identical player created over REST did — nothing errored, the timeline simply started later for some players than others. `ActivitiesRepository::announceAttendanceChange()` (eight callers) and #3081's cancellation fix are the same shape. Idempotency above is what makes this safe: the repository can fire unconditionally.
