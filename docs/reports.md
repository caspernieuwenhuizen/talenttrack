...
The **Reports** tile is a launcher for different ways of looking at your data. The reports are grouped by purpose so the right one is easy to find: **Development & performance** (ratings, progress, rate cards), **Playing time** (minutes played and squad load), **Attendance** (team and player attendance statistics and the leaderboard), **Recruitment** (scouting, prospects, trial funnel), **Staff & quality** (coach activity and evaluation quality), and **Season overview** (the annual review). Sections you don't have access to — recruitment and season-wide reports are academy-admin only — simply don't appear. The same holds tile by tile: the attendance reports, the leaderboard, minutes-per-team and the minutes audit need statistics access on top of reports access, so a role that may read reports but not statistics is not offered them at all. The launcher never shows you a report that would refuse you when you open it; if an attendance tile you expect is missing, ask an administrator to widen your access rather than following a link into a refusal. All standard reports — including team attendance, player attendance, the leaderboard and minutes-per-team — live here and breadcrumb under **Reports**; they are no longer duplicated on the Analytics dashboard.
...
- **One period control** — *Last week*, *This month* (month-to-date), *This season*, and **Custom range…**. *Last week* and *This month* are retrospective (they look back to today). ***This season* spans the whole season** — from the season's start date through the season's own **end date** — so it covers the entire campaign, not just the part that has happened so far. Picking a preset sets the window for you. **Custom range…** opens the From / To dates inside the same control, and typing there wins over the preset. Because it is one control, what it says is always the window the report actually used: pick a preset and it names the preset, type dates and it names the dates.
...
Your **saved views** sit in the filter bar itself, as chips at its right-hand end, on every standard report — the team, player and leaderboard attendance reports, and the minutes reports. Set the filters you keep coming back to (a team, an activity type, a period), click **Save current filters…**, give it a name like *"U17 league games"*, and it becomes a one-click chip you can re-apply any time. Saved views are **personal** — only you see yours — and each belongs to the one report you saved it on. A view remembers a period pill as a **relative** choice (*This season* stays "this season" next month too); a manual From/To range is remembered as the exact dates. Use the bookmark icon beside the chips to update, rename or delete one. Saving, listing and deleting run over the REST API (`GET/POST /filter-presets`, `DELETE /filter-presets/{id}`), gated on the capability of the surface the view belongs to — `tt_view_analytics` for these reports. The retired `/reports/filter-presets` paths still answer for one release.

Saved views are no longer reports-only. They are part of the shared filter bar, and are also available on the six standard reports and on the players, teams, people, evaluations, goals, tournaments, holidays and activities lists plus the audit log. On a list a saved view also remembers the **search term and the sort order**, not just the filters — restoring a view that reset your sort would not be the view you saved. Each screen's saved views are gated on that screen's own permission, so a saved view can never surface a screen you could not otherwise open. The attendance and minutes entry grids and the custom-fields settings screen deliberately do not offer them.
Your **saved views** sit in the filter bar itself, as chips at its right-hand end, on every standard report — the team, player and leaderboard attendance reports, and the minutes reports. Set the filters you keep coming back to (a team, an activity type, a period), click **Save current filters…**, give it a name like *"U17 league games"*, and it becomes a one-click chip you can re-apply any time. Saved views are **personal** — only you see yours — and each belongs to the one report you saved it on. A view remembers a period pill as a **relative** choice (*This season* stays "this season" next month too); a manual From/To range is remembered as the exact dates. The bookmark icon beside the chips opens a menu. **Update a view.** Open a saved view, change a filter (on the player report: a section, the order or the layout), and the menu's first action is **Update "‹name›"**: it stores the filters set now on the view you opened, keeping its name and whether it is your default. Until you change something there is nothing to update, so the action is not shown. Saving under the name of a view you already have on the same screen asks whether to replace that view's filters, rather than refusing; confirm and that view is updated, with no second view of the same name. The same name on a different screen, or used by someone else, is fine. **Rename, set as default or delete this view** opens a small dialog where you can rename the view, make it your default, or delete it. Renaming keeps the filters. Saving, listing and deleting run over the REST API (`GET/POST /filter-presets`, `DELETE /filter-presets/{id}`), gated on the capability of the surface the view belongs to — `tt_view_analytics` for these reports. The retired `/reports/filter-presets` paths still answer for one release. Saved views are no longer reports-only: they are part of the shared filter bar, so any view built on it can offer them.
Your **saved views** sit in the filter bar itself, as chips at its right-hand end, on every standard report — the team, player and leaderboard attendance reports, and the minutes reports. Set the filters you keep coming back to (a team, an activity type, a period), click **Save current filters…**, give it a name like *"U17 league games"*, and it becomes a one-click chip you can re-apply any time. Saved views are **personal** — only you see yours — and each belongs to the one report you saved it on. A view remembers a period pill as a **relative** choice (*This season* stays "this season" next month too); a manual From/To range is remembered as the exact dates. **Open one by default.** In that dialog, tick **Open this view by default on this screen** and that view is applied whenever you open the screen without filters of your own — one click becomes none. One default per screen per person; marking a new one releases the old. The default chip is marked with a star, and the address bar shows the filters that were applied, so the page stays bookmarkable and shareable. Your default never overrides a deliberate choice: a link that already carries filters, a **← Back to** pill, or a shared URL all show exactly what the address asks for. Use **Clear** in the filter bar to see everything unfiltered — that escapes the default for the visit rather than bouncing back into it. Currently available on the attendance and minutes reports.

The bookmark icon beside the chips opens a menu. **Update a view.** Open a saved view, change a filter (on the player report: a section, the order or the layout), and the menu's first action is **Update "‹name›"**: it stores the filters set now on the view you opened, keeping its name and whether it is your default. Until you change something there is nothing to update, so the action is not shown. Saving under the name of a view you already have on the same screen asks whether to replace that view's filters, rather than refusing; confirm and that view is updated, with no second view of the same name. The same name on a different screen, or used by someone else, is fine. **Rename, set as default or delete this view** opens a small dialog where you can rename the view, make it your default, or delete it. Renaming keeps the filters. Saving, listing and deleting run over the REST API (`GET/POST /filter-presets`, `DELETE /filter-presets/{id}`), gated on the capability of the surface the view belongs to — `tt_view_analytics` for these reports. The retired `/reports/filter-presets` paths still answer for one release. Saved views are no longer reports-only: they are part of the shared filter bar, so any view built on it can offer them.
...
**Plain or nested filters — and what a filter the endpoint cannot use does.** All three endpoints take `team_id`, `from`, `to` and `activity_type_key` either plainly, as above, or nested as `filter[team_id]`, `filter[from]` (also spelled `filter[date_from]`), `filter[to]` (`filter[date_to]`) and `filter[activity_type_key]` — the form the rest of the list API uses. The two spellings ask the same question, and a nested value wins when both are sent. Send a `team_id` that is not a usable team id and the request is refused with `400 bad_filter`, whose `details.parameter` names the spelling at fault; it is never dropped, because dropping it used to answer with every team in the academy under a filter somebody had asked for. Team scope still applies after the filter resolves: a team you may not read answers `403`, not your own teams' players.
