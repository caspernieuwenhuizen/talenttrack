...
4. Voor échte wissing: hard-delete de speler (Spelerslijst → bulkacties → Permanent verwijderen, alleen voor beheerders; er wordt eerst automatisch een back-upsnapshot gemaakt). Sinds v4.20.104 cascadeert de hard-delete door **elke** speler-gekoppelde tabel — evaluaties met hun beoordelingen, doelen inclusief discussiethreads, reisgebeurtenissen, blessures, aanwezigheid, POP-dossiers met gesprekken en oordelen, proefdossiers met stafinput, wedstrijd- en toernooireferenties, rapporten, gedrags-/potentieelregistraties. Wedstrijdscore-events en de prospect-conversielink behouden hun rij maar wissen de spelerreferentie. Twee dingen blijven handmatig: de spelersfoto (een URL naar `wp-content/uploads/` — verwijder het bestand via de Mediabibliotheek) en een eventueel gekoppeld WP-gebruikersaccount (wp-admin → Gebruikers). Documenteer het wisbesluit en de datum.
5. Log de aanvraag en jouw reactie in het privacyregister van je academie.
