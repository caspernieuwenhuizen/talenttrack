...
- **Hoofdtrainers stellen het potentieel in voor hun eigen selecties (#3967).** Een goedgekeurde verruiming van de toegang, na de brug: `head_coach` houdt `player_potential` `rc` met **team**-bereik (was `r`). Alleen de matrix regelt dit: trainers hebben de WordPress-rol `tt_coach`, die de personaresolver opsplitst in hoofd- en assistent-trainer, dus geen ruwe roltoekenning kan dat onderscheid maken. Het teambereik doet de afbakening — het profiel, het scherm Gedrag & potentieel en `POST /players/{id}/potential` vragen allemaal `PlayerStatusModule::potentialCaptureAvailableFor()`, dat is de capability plus `AuthorizationService::canEditPlayer()` voor die speler, zodat een hoofdtrainer één selectie verderop wordt geweigerd. `assistant_coach` verandert niet en houdt geen `change` op `player_potential` (#1060). Migratie `0287_authorization_seed_topup_head_coach_potential` voegt die ene regel toe op bestaande installaties (idempotente `INSERT IGNORE`; een door een beheerder aangepaste regel blijft ongemoeid). De melding *Potentieel niet herzien* volgt vanzelf: het publiek is wie de capability heeft, dus een hoofdtrainer krijgt hem nu voor de eigen teams.
