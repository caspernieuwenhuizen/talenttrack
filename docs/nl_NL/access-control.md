...
**Doel-threads volgen dezelfde regel.** Een verzorger neemt alleen deel aan het gesprek over de doelen van het kind zolang `ParentChildResolver::isParentOf()` zegt dat hij de verzorger van dat kind is. Is het kind vertrokken, gearchiveerd of in de prullenbak gezet, dan kan de verzorger in die threads niet meer lezen en niet meer schrijven, en krijgt hij geen meldingen meer van nieuwe berichten. Er is geen tussenvorm van "wel lezen, niet schrijven": een afgesloten gezin houdt op geen enkel scherm gedeeltelijke toegang. Het kind herstellen of weer actief maken herstelt het gesprek. `PlayerParentsRepository::parentsForPlayer()` somt elke ooit gelegde koppeling op en is bedoeld voor het beheren van koppelingen; of iemand als verzorger mag optreden is altijd het antwoord van de resolver.
