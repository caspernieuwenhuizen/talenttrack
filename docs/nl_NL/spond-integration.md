...
- **Activiteitstype** — TalentTrack's keyword-classificator kiest training, wedstrijd, toernooi of vergadering. **De titel bepaalt.** De meeste trefwoorden mogen ergens in de titel staan, dus "Thuiswedstrijd" en "Trainingswedstrijd" worden allebei een wedstrijd. De twee korte trefwoorden die ook ín langere Nederlandse woorden zitten — `kamp` (Noors voor *wedstrijd*, want Spond is een Noors product) en `uit` — tellen alleen als **heel woord**: "Kamp mot Rosenborg" is een wedstrijd, terwijl "Trainingskamp" en "Voetbalkamp" trainingen zijn.

  De **omschrijving is een terugvaloptie, geen tweede titel**. Die wordt alleen gelezen als er in de titel géén enkel herkend trefwoord staat. Een training met de titel "Training JO14-1" en de omschrijving *"laatste training voor de wedstrijd van zaterdag"* blijft dus een **training** — een zin over het weekend maakt van de woensdagsessie geen wedstrijd. Een activiteit die alleen "JO14-1" heet en als omschrijving *"wedstrijd tegen Ajax"* heeft, wordt wél een wedstrijd, want de titel zei niets anders. In de omschrijving gelden dezelfde heel-woordregels.
