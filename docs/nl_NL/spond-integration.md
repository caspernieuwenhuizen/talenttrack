...
- **Locatie (naam + adres)** — heeft de locatie van een Spond-event zowel een naam als een straatadres, dan bewaart TalentTrack **beide** — de naam op de eerste regel, het adres op de tweede. (De Week-PDF drukt de naam inline af en het adres op de regel eronder.) Omdat locatie een "Spond wint"-veld is, wordt een aanpassing in TalentTrack bij de volgende sync overschreven; het bewerkformulier heeft één regel en kan de tweede regel niet vasthouden, dus opnieuw opslaan vouwt hem samen — de volgende sync herstelt beide regels.
