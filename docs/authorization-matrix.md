...
- **Head coaches set potential for their own squads (#3967).** An approved access widening, made after the bridge: `head_coach` holds `player_potential` `rc` at **team** scope (it held `r`). The matrix carries this alone: coaches are the `tt_coach` WordPress role, which the persona resolver splits into head and assistant coach, so no raw role grant could draw that line. Team scope does the narrowing — the profile, the Behaviour & potential screen and `POST /players/{id}/potential` all ask `PlayerStatusModule::potentialCaptureAvailableFor()`, which is the capability plus `AuthorizationService::canEditPlayer()` for that player, so a head coach is refused one squad over. `assistant_coach` is unchanged and holds no `player_potential` change (#1060). Migration `0287_authorization_seed_topup_head_coach_potential` adds the one tuple on existing installs (idempotent `INSERT IGNORE`; an admin-edited row is left alone). The *Potential not revisited* alert follows automatically: its audience is whoever holds the capability, so a head coach is now told about their own teams.
