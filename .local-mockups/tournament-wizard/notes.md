...
| 3 | Squad checkbox = binary in/out, easy to lose context | **All players Present by default**; each row carries an attendance-style chip strip (Present / Absent / Injured / Excused) mirroring the mark-attendance wizard pattern. Non-Present rows dim + line-through but stay visible so the coach can revert without re-searching. Mark-all-present bulk action; live count "X in squad · Y out". |
| 3a | No way to add a guest player to the squad | New **+ Add guest** button below the team roster opens a modal mirroring the activity guest-add flow. Two tabs: **From another team** (search + pick a player from any non-anchor team in the club) and **Anonymous** (free-text name + optional notes). Both panes carry position-eligibility chips. Added guests append at the bottom of the squad list with a `Guest` badge and the same attendance + position chip rows as roster players. |
| 4 | Position chips look identical to non-position chips | Active position chips fill with the accent colour. Inactive look like outline buttons. Position-type "GK/DEF/MID/FWD" stays as the v1 spec (NOT individual position codes — per the v1 shaping decision). Position chips become non-interactive on rows whose attendance status is non-Present (they don't enter the planner's auto-balance pool anyway). |
...
- **Squad uses the attendance-chip pattern, not a binary checkbox** (refined 2026-05-28 per pilot feedback). All team players default Present; each row carries a 4-chip strip (Present / Absent / Injured / Excused) mirroring the `assets/css/attendance-cards.css` look from the mark-attendance wizard. Non-Present rows dim + line-through but stay in the list so the coach can revert. Position-eligibility chips below the attendance strip remain — they're disabled on non-Present rows since those players don't enter the planner's auto-balance pool.
- **Guest-add reuses the activity guest-add modal's two-tab pattern** (linked from another team / anonymous), restyled to the wizard's card+chip aesthetic. Added guests append at the bottom of the squad list with a `Guest` badge and the same attendance + position chip rows. The "From another team" pane has a **Team dropdown + name search** side-by-side — the dropdown lists every non-anchor team in the club ("All other teams" default; anchor team is excluded automatically); the name input filters the visible result list live. Both filters AND together — pick a team to narrow, then type to refine. The "Anonymous" pane is bare name + position chips + optional notes — no `tt_players` record is created, the guest is recorded against this tournament only.
...
5. ~~**Anchor team's roster source** — squad step pulls from the picked team's active players. Trial players (`status='trial'`) currently render with a "trial" sub-line and unchecked by default. **Confirm: keep trial players visible but unchecked, or filter them out entirely?**~~ **Resolved 2026-05-28**: all team players (active + trial) appear in the squad list and default to Present. The trial-status meta sub-line stays; the coach toggles a trial player Absent / Excused if they're not joining. Same pattern as the mark-attendance wizard.
