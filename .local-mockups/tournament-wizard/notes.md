...
- **Guest-add reuses the activity guest-add modal's two-tab pattern** (linked from another team / anonymous), restyled to the wizard's card+chip aesthetic. Added guests append at the bottom of the squad list with a `Guest` badge and the same attendance + position chip rows. The "From another team" pane is a search-and-pick over the club's non-anchor-team players; the "Anonymous" pane is bare name + position chips + optional notes — no `tt_players` record is created, the guest is recorded against this tournament only.
