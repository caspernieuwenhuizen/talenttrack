 ...
 *
 * #0085 follow-up — backfill `tt_authorization_matrix` with the new
 * `player_notes` matrix entity introduced in v3.95.2:
 *
 *   - assistant_coach / head_coach / team_manager: r/c[team]
 *   - scout: r/c[global]  (cross-team scouting workflow)
 *   - head_of_development / academy_admin: r/c/d[global]
 *   - player + parent: no grant
 *
 * Same pattern as 0063 / 0064 / 0067: walk the seed file and INSERT
 * IGNORE every (persona, entity, activity, scope_kind) tuple. Existing
 * rows including operator-edited ones stay untouched; only the new
 * tuples land. Per `feedback_seed_changes_need_topup_migration.md`:
...
return new class extends Migration {
    ...
    public function getName(): string {
        return '0069_authorization_seed_topup_player_notes';
    }
    ...
    public function up(): void {
        ...
        foreach ( $rows as $row ) {
            if ( ( $row['entity'] ?? '' ) !== 'player_notes' ) continue;
