...
**2. Translations shipped in the same PR, as a fragment.**
Any change that adds, renames, or removes a `__()` / `_e()` / `esc_html__()` string must ship its Dutch in the same PR — but **not** by editing `languages/talenttrack-nl_NL.po`. Drop a fragment instead: `languages/pending/<issue>-<slug>.po`, carrying only the entries this PR introduces, with their `msgstr` filled in. It is the same pattern `changelog.d` uses, for the same reason — a brand-new file per PR never collides with another branch's (#3862). The release consolidates the fragments into the catalogues and deletes them. Don't regenerate `talenttrack.pot` in a feature PR either, and never commit a `.mo` — the release owns both (see § "The translation catalogue").
...
**Enforced by `docs-lint.yml`** (#2551). It checks that every doc is registered or explicitly dev-only, that `group` / `audience` / `module` / `feature` / `tier` / `capability` resolve, that every routable `?tt_view=` slug is claimed by a topic or listed in `config/no_help_topic.php`, that cross-references and `?tt_view=` links point at things that exist, that no reader-facing topic gains a version stamp or issue number, and that every file is valid UTF-8. Run it locally with `php tools/check-docs.php --base=origin/main`; the rules are documented in `docs/contributing.md`. Label a PR `docs-lint-exempt` for a deliberate exception.
