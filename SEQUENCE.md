...
| 0052 | SaaS-readiness baseline — one-time backfill bringing the existing codebase up to the standard codified in `CLAUDE.md` (tenancy scaffold, REST coverage, auth-portability, asset-portability). Audit complete on v3.39.0; ~15 tenant-scoped tables need `club_id` + root entities need `uuid`; 41 files still post to `admin_post_*` / `wp_ajax_*`. Idea: [ideas/0052-epic-saas-readiness-baseline.md](ideas/0052-epic-saas-readiness-baseline.md). | epic | Sprint sequencing across 38 migrations and 20+ modules; how aggressive on the `admin_post_*` migration (port-on-touch vs sweeping); whether to publish OpenAPI spec as part of this epic or a follow-up | ~4-6 sprints |
| 0053 | Player journey — make every player's path through the academy first-class. New `tt_player_events` table + transition types (trial→signed, age-group promotion, position change, injury→return, release, graduation) + unified timeline read model + journey tab on the player profile. Aligns with `CLAUDE.md` § 1. ~70% of source data already exists across evaluations / goals / PDP verdicts / trial admissions. Idea: [ideas/0053-epic-player-journey.md](ideas/0053-epic-player-journey.md). | epic | What counts as a journey event (cut line vs noise from session attendance); whether `tt_audit_log` has any role here; injury data shape + privacy posture; transition vocabulary lock; how the journey relates to PDP cycles | ~4-5 sprints |
...
| - | - | - | - | - | - |
| 0055 | Record-creation wizards — full epic in one PR. Five-primitive framework under `src/Shared/Wizards/` (`WizardInterface`, `WizardStepInterface`, `WizardRegistry`, `WizardState` transient store, `WizardAnalytics` `wp_options` counters) + generic driver at `?tt_view=wizard&slug=<…>`. Four shipped wizards: `new-player` (trial-vs-roster branching with auto-creation of a real `tt_trial_cases` row via #0017), `new-team` (basics → staff slots → review, mapping each filled slot to a `tt_team_people` row via `FunctionalRolesRepository::findRoleByKey`), `new-evaluation` (player → type → handoff to existing eval form pre-filled), `new-goal` (player → polymorphic methodology link → details, writes goal + optional `tt_goal_links` row). Entry-point gating one-liner via `WizardEntryPoint::urlFor()` on Players/Teams/Evaluations/Goals manage views; `tt_wizards_enabled` config (`all` / `off` / CSV) defaults to `all`. Phase-4 admin at `?tt_view=wizards-admin` with completion analytics + per-step skip tracking. Setup-wizard hook on `OnboardingPage::renderFirstTeam` linking to the frontend new-team wizard. Mobile-first inline CSS, help-topic sidebar per wizard. | epic | v3.43.0 | ~32-46h across 4 phases | _TBD_ |
