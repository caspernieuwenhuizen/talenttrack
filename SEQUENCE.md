...
| - | - | - | - | - | - |
| 0051 | Module surface gating — disabled modules now actually disappear from the UI. `Modules` admin toggle previously stopped a module from booting (REST routes, hooks, capabilities) but left frontend tiles + wp-admin sidebar entries + wp-admin dashboard tiles still rendering, and direct URLs still resolved. New `src/Core/ModuleSurfaceMap.php` is a single declarative `tt_view`/`?page=` slug → module-class lookup; `FrontendTileGrid::renderGroups()`, `DashboardShortcode::render()`, `Menu::register()` + `Menu::dashboard()`, and `MenuExtension::register_submenu()` all consult it. Disabled-module direct URL hits get a friendly "this section is currently unavailable" notice + back button instead of a 404 / fatal. Always-on modules (Auth/Configuration/Authorization) pass through unconditionally. Closes the long-standing #0033 Sprint 4 acceptance criterion ("no tile literals leak modules' surfaces"). | feat | v3.35.0 | ~4-6h spec | ~2h actual |
