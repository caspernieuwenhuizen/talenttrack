...
| 7 | **#0019 Sprint 3** — players + teams frontend | epic | ~22h | ~10h so far | IN PROGRESS | Shaped into 2 sessions (vs spec's 4) per Casper's preference. v3.9.0 = session 3.1 (Players full frontend: list+CRUD+photo+custom fields+rate-card link). v3.9.1 = session 3.2 (Teams full + roster + sync CSV import — spec's async per-row dupe UI dropped per Q1). |
...
- **Session 3 (v3.7.2, ~3h)** — `assets/css/frontend-admin.css` design-token + component stylesheet, loaded alongside the legacy `public.css`. Five shared components under `src/Shared/Frontend/Components/`: `FormSaveButton` (idle/saving/saved/error state machine driven by public.js), `PlayerPickerComponent`, `DateInputComponent`, `RatingInputComponent` (with dot-track sync in `components/rating.js`), `MultiSelectTagComponent` (with tag-picker JS in `components/multitag.js`). Flash-message JS layer at `assets/js/components/flash.js` — animates server-rendered banner dismissal, auto-fades success after 5s, exposes a `window.ttFlash.add(type, message)` helper for JS-side pushes. `assets/js/drafts.js` — opt-in via `data-draft-key`, debounced localStorage snapshot, 14-day TTL, restore prompt rendered above the form on load, cleared on successful `tt:form-saved` event. Coach eval/session/goal forms wired up to both FormSaveButton and drafts.
