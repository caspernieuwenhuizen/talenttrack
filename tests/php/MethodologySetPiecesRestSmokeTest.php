...
/**
 * REST smoke suite for the methodology set-pieces authoring endpoints
 * (#2228). Parallels MethodologyPrinciplesRestSmokeTest — the routes live
 * at /talenttrack/v1/methodology/set-pieces and gate on the
 ...
 *       every verb, never 200 and never >=500.
 *   (b) Happy path + envelope — an administrator can create → get → update
 *       → delete a club-authored set piece, asserting status + the
 *       success / data / errors envelope, plus that shipped rows refuse
 *       edit/delete with a 409.
 */
final class MethodologySetPiecesRestSmokeTest extends WP_UnitTestCase {

    private const BASE = '/talenttrack/v1/methodology/set-pieces';
     ...
     */
    public function test_set_piece_crud_happy_path(): void {
        ...
        $create->set_body( wp_json_encode( [
            ...
            'bullets'  => [ 'nl' => [ 'Loop naar de tweede paal' ], 'en' => [ 'Run to the far post' ] ],
        ] ) );
        $create_res = rest_do_request( $create );

        $this->assertContains( $create_res->get_status(), [ 200, 201 ], 'set-piece create succeeds' );
        $body = $create_res->get_data();
        $this->assertEnvelopeSuccess( $body );
        $id = (int) ( $body['data']['id'] ?? 0 );
        $this->assertGreaterThan( 0, $id, 'create returns a new set-piece id' );
        ...
        $this->assertSame( [ 'Run to the far post' ], $get_body['data']['bullets_i18n']['en'] ?? null, 'English bullets round-trip' );
        ...
        $this->assertSame( 200, $update_res->get_status(), 'set-piece update succeeds' );
        $this->assertEnvelopeSuccess( $update_res->get_data() );

        // DELETE.
        $delete_res = rest_do_request( new WP_REST_Request( 'DELETE', self::BASE . '/' . $id ) );
        $this->assertSame( 200, $delete_res->get_status(), 'set-piece delete succeeds' );
        $delete_body = $delete_res->get_data();
        $this->assertEnvelopeSuccess( $delete_body );
        $this->assertTrue( (bool) ( $delete_body['data']['deleted'] ?? false ), 'delete removes the set piece' );
    }

    /**
     * A malformed authorized create (invalid kind) returns a 400 with the
     * error envelope, not a 500 or a silent success.
     */
    public function test_set_piece_create_invalid_kind_is_bad_request(): void {
        $uid = self::factory()->user->create( [ 'role' => 'administrator' ] );
        wp_set_current_user( $uid );
        ...
        $this->assertSame( 400, $res->get_status(), 'invalid kind is a 400' );
        $body = $res->get_data();
    ...
    /**
     * Shipped set pieces are read-only reference content: update and delete
     * both refuse with a 409, never mutating the row.
     */
    public function test_shipped_set_piece_refuses_edit_and_delete(): void {
        $uid = self::factory()->user->create( [ 'role' => 'administrator' ] );
        wp_set_current_user( $uid );

        global $wpdb;
        $wpdb->insert( $wpdb->prefix . 'tt_set_pieces', [
            'club_id'    => 1,
            'slug'       => 'shipped-corner',
            'kind_key'   => 'corner',
            'side'       => 'attacking',
